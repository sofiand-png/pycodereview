...
## 6. Non-deviable Rules in Strict

Certain rules are **non-deviable** in the Strict profile. Any violation of these rules
is considered a non-compliance and cannot be waived by deviation, except under
extraordinary, project-level approval.

Examples include:

- Dangerous dynamic features in Strict code (e.g. dynamic imports and monkeypatching).  
- Use of `Any` in Strict-critical units.  
- Hardcoded secrets and obviously insecure APIs.  
- Nondeterministic randomness in critical logic.

Projects shall document their list of non-deviable rules in the GCS.

## 7. 🔒 Interpretation Note — Phase-Aware Enforcement (Strict)

In the **Strict profile**, all rules that ban:

- I/O (file, network, database, IPC),
- blocking operations,
- subprocesses,
- networking,
- dynamic allocation,
...
in **core computational logic** or **critical execution paths** **MUST** be enforced strictly on:

1. Any function or method annotated `@critical`, and  
2. Any unit explicitly classified as critical core logic in the **Mode Assignment Register (MAR)**.

---

### ✅ Rule Groups Affected

The model applies to three categories of rules:
...
#### ✅ Strict-Strengthened Core Rules
Rules that become phase-sensitive under Strict:
...
#### ✅ Indirect Timing/Determinism Rules

- CRSS-11.4.8
- CRSS-11.6.1

**Note:** This list is normatively binding but not exhaustive. Tools and reviewers **MAY** classify additional rules as critical-phase applicable if they clearly constrain:
...
### ✅ What Strict @critical Means

Inside a Strict `@critical` phase, the following are **forbidden**:

- ❌ File/network/DB/IPC I/O  
- ❌ Blocking operations (sleep, join, locks, blocking I/O)  
- ❌ Subprocess or shell invocation  
- ❌ Dynamic memory allocation that may trigger GC  
- ❌ Any construct reliant on scheduling or wall-clock timing  
- ❌ Calling non-critical code that performs any of the above  

**In short:**

> **Strict + @critical = pure, bounded, deterministic computation only.**

---

### ✅ What Strict Non-Critical Means

In Strict non-critical units (including `@non_critical_phase`):
...
❗ Still subject to all Strict-profile rules, including:

- network robustness (7.8.x / 7.9.x)
- bounded data and caches (5.4.x, 5.6.x, 7.8.x)
- security & sensitive data (6.4.x)
- OS/environment safeguards (7.10.x)

And critically:

- ❌ Strict MUST-NOT rules are NEVER waived in non-critical code.

Non-critical code may run:

- ✅ before a `@critical` call, and  
- ✅ after a `@critical` call  
...
The critical phase is defined **per call**, not per file:

- Entering a `@critical` function → entering critical phase
- Returning → exiting critical phase

The caller remains non-critical **as long as it does not implement critical logic itself**.

---

### ✅ Valid Strict Pattern

```python
def main_loop():  # non-critical Strict-B
    inputs = read_inputs()              # @non_critical_phase
    decision = controller.decide(...)   # @critical
    publish_decision(decision)          # @non_critical_phase
```

## 8. Summary

CRSS-Python Strict represents a **constrained, analyzable subset of
Python** suitable for high-integrity components. In practice:

- Projects may use `crss-core` for general code, and
- `crss-strict` for **critical zones** and `@critical` units.

This profile is intended as a foundation for future static and dynamic
tooling, including:

- MC/DC analyzers,
- path coverage exploration,
- automated compliance reporting against CRSS rule IDs.
